    ...
    <footer className="bg-slate-900 py-10 text-slate-100">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:grid-cols-3 sm:px-6 lg:px-8">
        ...
        <div>
          ...
          <p className="mt-3 text-sm text-slate-300">Follow us on social media for the latest drops and exclusive offers.</p>
          <div className="mt-4 flex gap-3">
            {['twitter', 'instagram', 'facebook'].map((network) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-full bg-white/10 p-3 text-white transition hover:bg-primary"
              >
      ...
      <p className="mt-10 text-center text-xs text-slate-500">© {new Date().getFullYear()} Modern Store. All rights reserved.</p>
    </footer>
  );
}
